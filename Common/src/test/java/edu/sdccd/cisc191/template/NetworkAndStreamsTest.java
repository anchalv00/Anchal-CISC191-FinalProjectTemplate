...
public class NetworkAndStreamsTest
{
    ...
    /*
     * Uses a try-catch to connect a client to a server and create its output/input streams
     * Writes an object over the stream and is read and tested to see if the contents are accurate
     */
    @Test
    public void testNetworking() throws IOException {
        try (Socket socket = new Socket(InetAddress.getLocalHost().getHostName(), 4444)) {
            ...
            //CostumerRequest request = new CostumerRequest(1);
